...
| v7 | 2026-07-26 | 编排层落地(RecursiveExtractor 58 文件变更共 +4173/-1388 行)、标题感知分块(StoreWriter.writeHierarchicalChunks)、Ole10Native完整格式修复(OleExtractor: 正确跳过 srcPath+tmpPath)、DocHandler全功能重写(表格+图片+标题+OLE+页眉+删除线+独立unpack, 63→331 行)、图片原位内联(DocxHandler extractParagraph 按 run 顺序交替输出 image/paragraph Element)、删除线标记(~~...~~, DocxHandler+DocHandler)、PDF Y坐标图文混排(PdfHandler YCoordinateStripper)、Phase 1.5可插拔接口(TriageProcessor+ImageDescriber+NoOp 默认实现)、MergeCellResolver FormulaEvaluator 缓存(每 Workbook 一个实例)、ThreadLocal<DataFormatter> 替换为局部变量、配置前缀统一(extractor.*)、ExcelHandler 单列表头检测修复(findHeaderRowIndex 自适应阈值)、LargeTableInfo.clearRows() CSV 写出后释放内存、escapeJson 完整 JSON 控制字符转义、hwpfTableToMarkdown 分隔行条件改为 r==0、DocxHandler detectHeadingLevel \"Heading\"无数字返回0、buildParagraphText 死代码清理、SPEC.md(v7)+AGENTS.md(源码结构/路线图)更新 |
